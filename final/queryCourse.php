...
function getSomeTag($num = 10){
	require('../../mysqli_connect.php');
	$TagArr = array();
	$query = "select distinct tag from CourseTag";
	$r = @mysqli_query($dbc, $query);
	while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)){
		array_push($TagArr, $row['tag']);
//	echo $row['code'].'<br>';
	}
	mysqli_free_result($r);
	return $TagArr;
...
//		print $selCourse.'<br>';
	...
	$resultArr = array();
	foreach ($resultCourse as $h => $v){
		if ($h != '40100600' && $h != '40101300' && $h!='40100800' ){
			$resultArr[$h] = $v;
		}
	}
	$resultCourse = $resultArr;
...
function getRecomCourseWithTag($TagArr, $num = 5){
	require('../../mysqli_connect.php');
	$query = "select distinct code from CourseTag where ";
	foreach ($TagArr as $h => $v){
		$query = $query."tag = '$v' or ";
	}
	$query = $query." 1 = 2;";
	$r = @mysqli_query($dbc, $query);
	$codeArr = array();
	...
	mysqli_free_result($r);
	$codeArr = array_slice($codeArr, 0, $num);
	return $codeArr;
}
