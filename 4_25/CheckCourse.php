<?php
include_once 'jwbManager.php';

echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
class CheckCourse{
	public function check($u){
		$this->reCoDmArr = array();
		$this->reCoMcArr = array();
		$this->seLBMcArr = array();
		$this->seXfNeedArr = array();
		$this->recomArr = array();
		$jwb = new jwbManager();
		//Get Course you had passed
		$CCodeArr = $jwb->getCourseCode($u);
	...
	//	echo $dbc;
		...
		foreach ($kcdmarr as $h1 => $v1){
			if ($lbarr[$h1] == '0'){
				...
				if (!$flag){
					...
					array_push($this->reCoDmArr, $v1);
					array_push($this->reCoMcArr, $kcmcarr[$h1]);
				}
			}
		...
		// echo selective courses
		foreach($lbarr as $h => $v){
			if (!isset($lbxf[$lbarr[$h]])){
				$lbxf[$lbarr[$h]] = 0;
			}
	...
	//			echo 'constraint is: '.$xfxzarr[$h].' Yours is:'.$lbxf[$lbarr[$h]].'<br>';
		//		echo 'The course part: '.$lbmcarr[$h].' is not full! It need more '.($xfxzarr[$h] - $lbxf[$lbarr[$h]]).'<br>';
				array_push($this->seLBMcArr, $lbmcarr[$h]);
				array_push($this->seXfNeedArr, ($xfxzarr[$h] - $lbxf[$lbarr[$h]]));
			}
		}

	}

	public function getReCoDmArr(){
		return $this->reCoDmArr;
	}
	public function getReCoMcArr(){
		return $this->reCoMcArr;
	}
	public function getSeLBMcArr(){
		return $this->seLBMcArr;
	}
	public function getSeXfNeedArr(){
		return $this->seXfNeedArr;
	}

	private $reCoDmArr = array();
	private $reCoMcArr = array();
	private $seLBMcArr = array();
	private $seXfNeedArr = array();
	private $recomArr =  array();
}
?>
